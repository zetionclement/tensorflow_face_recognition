...
# 以当天日期为表名创建一张用于记录识别成功的表
def save_success():
    db1 = pymysql.connect('127.0.0.1','root','123456','success_recognition',charset='utf8')
    cursor1 = db1.cursor()
    sql = """CREATE TABLE IF NOT EXISTS %s""" % (time.strftime('%Y_%m_%d', time.localtime(time.time())))
    sql += """ (name VARCHAR(50) NOT NULL UNIQUE,time DATETIME NOT NULL,PRIMARY KEY (name))ENGINE=InnoDB DEFAULT CHARSET=utf8MB4;"""
    try:
        cursor1.execute(sql)
    except:
        print("表%s已创建"%(time.strftime('%Y_%m_%d', time.localtime(time.time()))))
    db1.close()


def known_faces(feature_known_list,name_list):
    db = pymysql.connect('127.0.0.1','root','123456','person_embeddings_db',charset='utf8')
    cursor = db.cursor()
    sql = """SELECT * FROM person_average_embeddings;"""
    try:
        cursor.execute(sql)
    ...
    for i in range(len(resutls)):
        embeddings = list(resutls[i])
        name_list.append(embeddings.pop(0))
        feature_known_list.append(embeddings)

    db.close()
    return feature_known_list,name_list
...
#人脸识别
def face_recognition(image,bounding_boxes,feature_known_list,pos_namelist,print_namelist,name_list,db1,cursor1):

    del pos_namelist[:] # 人脸名字的坐标，每次用的时候先清空
    del print_namelist [:] # 人脸名字，每次用的时候先清空
    ...
    for k in range(len(features_cap_list)):
        ...
        if person_euclidean_list[index] <= 0.7:  # 即使找到一个最相似的脸，也要设定一个阀值（根据实际情况自行设定），只有低于这个阀值时才能认为是同一个人
            print(person_euclidean_list[index])
            print_namelist[k] = str(name_list[index])
            cv2.rectangle(image,(bounding_boxes[k][0],bounding_boxes[k][1]),(bounding_boxes[k][2],bounding_boxes[k][3]),(0,255,0),2)  #在图片上用矩形框人脸
            image=put_text(image,str(name_list[index]),pos_namelist[k])     #在图片上打印名字
            sql = """INSERT INTO success_recognition.%s (name,time) VALUES(\'%s\',\'%s\');""" % (time.strftime('%Y_%m_%d', time.localtime(time.time())), str(name_list[index]),time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
            try:
                cursor1.execute(sql)
                db1.commit()
            except:
                print("插入数据失败,可能数据已存在")

            # image = put_text(image, str(round(person_euclidean_list[index],2)), min_eculidean_position_list[k])   #在图片上打印欧士距离
        else:
            print(person_euclidean_list[index])
            cv2.rectangle(image, (bounding_boxes[k][0], bounding_boxes[k][1]),(bounding_boxes[k][2], bounding_boxes[k][3]), (0, 255, 255), 2)  #在图片上用矩形框人脸
            # image=put_text(image,str(round(person_euclidean_list[index],2)),min_eculidean_position_list[k])    #在图片上打印欧士距离
    ...
    cv2.waitKey(1)
    print("\n")
    print("屏幕中的人脸为:", print_namelist,"\n")


# 打开摄像头保存帧
def save_frame(images_que, pos_namelist, print_namelist,name_list, open_time):
    url = 'rtsp://admin:root123456@192.168.1.104:1554//Streaming/Channels/1'       #这里采用的是海康威视的ip摄像头
    ...
    while True:
        ...
        if ret:
            ...
            # cv2.waitKey(1)
            temp += 1
            if temp == 4:      #这里设定每22帧就保存1帧，如果每一帧都要进行人脸识别的话可能会卡顿，可以根据自己的实际情况设定
                #print("保存一帧")
...
# 人脸检测和人脸识别
def face_check(images_que, feature_known_list, pos_namelist, print_namelist,name_list):
    '''
    pid2 = os.getpid()
    f = open("info.txt", 'a')
    f.write('p2:' + str(pid2) + "\n")
        ...
        with tf.Session()as sess:
            ...
            while True:
                ...
                if faces_num != 0:
                    ...
                    face_recognition(image, bounding_boxes, feature_known_list, pos_namelist,print_namelist,name_list,db1,cursor1)  # 如果有人脸就调用人脸识别函数
                else:
                    cv2.namedWindow('frame', cv2.WINDOW_NORMAL)
                    cv2.resizeWindow('frame', 1280, 720)
                    cv2.imshow('frame', image)
...
def main_process():
    ...
    with Manager() as manager:
        feature_known_list = manager.list()  # 数据库中的人脸的特征list
        name_list = manager.list() # 数据库中的人脸名字
        pos_namelist = manager.list()  # 要在屏幕上打印的人脸名字的坐标
        print_namelist = manager.list()  # 要在屏幕上打印的人脸名字
        feature_known_list,name_list = known_faces(feature_known_list,name_list)  # 遍历所有已知的人脸数据
        save_success()
        images_que = Queue()         #用来保存从摄像头拍到的帧
        p1 = Process(target=save_frame, args=(images_que, pos_namelist,print_namelist, name_list,time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())),)) #进程1用于打开摄像头，并保存帧
        print("Create ProcessP1\n")
        p2 = Process(target=face_check,args=(images_que, feature_known_list, pos_namelist, print_namelist,name_list,))    #进程2用于人脸检测和人脸识别
